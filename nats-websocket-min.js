...
hexRand(0x0010000)+
hexRand(0x0010000)+
hexRand(0x1000000));};function Client(opts,listeners){EventEmitter.call(this);this.parseOptions(opts);this.addListeners(listeners);this.initState();this.createConnection();}
var connect=function(opts,listeners){return new Client(opts,listeners);};inherits(Client,EventEmitter);Client.prototype.createInbox=createInbox;Client.prototype.assignOption=function(opts,prop,assign){if(assign===undefined){assign=prop;}
...
if(options.pass===undefined){options.pass=auth[1];}}}};Client.prototype.addListeners=function(list){if('object'!=typeof list){console.error("listeners must be an Object");return;}
var events=['connect','disconnect','error','close']
for(e in events){if('function'==typeof list[events[e]])
this.addListener(events[e],list[events[e]])}}
Client.prototype.createConnection=function(){var client=this;client.pongs=[];client.pending=[];client.pSize=0;client.pstate=AWAITING_CONTROL;var scheme=(window.location.protocol||document.location.protocol)=="https:"?'wss://':'ws://';var stream=new WebSocket(scheme+window.location.host+client.options.path);client.stream=stream
...
client.flush(function(){client.emit(event,client);});};stream.onclose=function(evt){client.closeStream();client.emit('disconnect',evt.code&&evt.code>=4000?evt.code-4000:evt.code,evt.reason);if(client.closed===true||client.options.reconnect===false||client.reconnects>=client.options.maxReconnectAttempts){client.emit('close');}else{client.scheduleReconnect();}};stream.onerror=function(exception){client.closeStream();if(client.reconnecting===false){client.emit('error',exception);}
